...
<script>document.getElementById('f').onsubmit=async e=>{e.preventDefault();const nameEl=document.getElementById('name'),shopEl=document.getElementById('shop'),reviewEl=document.getElementById('review'),planEl=document.getElementById('plan'),msgEl=document.getElementById('msg');const b={name:nameEl.value.trim(),shop_domain:shopEl.value.trim(),review_url:reviewEl.value.trim(),plan:planEl.value};const r=await fetch('/api/businesses',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(b)});const d=await r.json();msgEl.textContent=r.ok?'Created. Business ID: '+d.id:('Error: '+(d.error||'failed'));}</script>
</body></html>`;
